...
UPDATE bookings
SET time_range = tstzrange(COALESCE(start_at, check_in::timestamptz), COALESCE(end_at, check_out::timestamptz), '[)')
...
CREATE OR REPLACE FUNCTION sync_booking_time_range()
RETURNS trigger AS $$
BEGIN
  NEW.time_range := tstzrange(COALESCE(NEW.start_at, NEW.check_in::timestamptz), COALESCE(NEW.end_at, NEW.check_out::timestamptz), '[)');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS bookings_sync_time_range ON bookings;
CREATE TRIGGER bookings_sync_time_range
BEFORE INSERT OR UPDATE OF start_at, end_at, check_in, check_out ON bookings
FOR EACH ROW EXECUTE FUNCTION sync_booking_time_range();
